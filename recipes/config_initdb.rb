...
# following node.default['rackspace_postgresql']['config'] attributes:
#
...
# etc. through these node.default['rackspace_postgresql']['config'] attributes:
#
# - Where to Log -
#   * log_destination = 'stderr'
#   * log_directory = 'pg_log'
#   * log_filename = 'rackspace_postgresql-%a.log'
...
node.default['rackspace_postgresql']['config']['datestyle'] = "iso, #{locale_date_order}"
...
node.default['rackspace_postgresql']['config']['lc_messages'] =
  [ENV['LC_ALL'], ENV['LC_MESSAGES'], ENV['LANG']].compact.first

node.default['rackspace_postgresql']['config']['lc_monetary'] =
  [ENV['LC_ALL'], ENV['LC_MONETARY'], ENV['LANG']].compact.first

node.default['rackspace_postgresql']['config']['lc_numeric'] =
  [ENV['LC_ALL'], ENV['LC_NUMERIC'], ENV['LANG']].compact.first

node.default['rackspace_postgresql']['config']['lc_time'] =
  [ENV['LC_ALL'], ENV['LC_TIME'], ENV['LANG']].compact.first

node.default['rackspace_postgresql']['config']['default_text_search_config'] =
  case ENV['LANG']
  when /da_.*/
    'pg_catalog.danish'
  when /nl_.*/
...
unless default_timezone.nil?
  node.default['rackspace_postgresql']['config']['log_timezone'] = default_timezone
  node.default['rackspace_postgresql']['config']['timezone'] = default_timezone
end

#######
# - Where to Log -
node.default['rackspace_postgresql']['config']['log_destination'] = 'stderr'
node.default['rackspace_postgresql']['config']['log_directory'] = 'pg_log'
node.default['rackspace_postgresql']['config']['log_filename'] = 'rackspace_postgresql-%a.log'
node.default['rackspace_postgresql']['config']['logging_collector'] = true # on
node.default['rackspace_postgresql']['config']['log_rotation_age'] = '1d'
node.default['rackspace_postgresql']['config']['log_rotation_size'] = 0
node.default['rackspace_postgresql']['config']['log_truncate_on_rotation'] = true # on
