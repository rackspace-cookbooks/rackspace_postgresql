...
# node['rackspace_postgresql']['config_pgtune'] attributes:
...
# node.default['rackspace_postgresql']['config'] attributes:
#   * max_connections
#   * shared_buffers
...
# that can be specified as node['rackspace_postgresql']['config_pgtune']['db_type']:
#
# dw -- Data Warehouse
...
if node['rackspace_postgresql'].attribute?('config_pgtune') && node['rackspace_postgresql']['config_pgtune'].attribute?('db_type')
  db_type = node['rackspace_postgresql']['config_pgtune']['db_type']
  unless %w['dw', 'oltp', 'web', 'mixed', 'desktop'].include?(db_type)
    Chef::Application.fatal!([
        "Bad value (#{db_type})",
        'for node[\'rackspace_postgresql\'][\'config_pgtune\'][\'db_type\'] attribute.',
...
if node['rackspace_postgresql'].attribute?('config_pgtune') && node['rackspace_postgresql']['config_pgtune'].attribute?('max_connections')
  max_connections = node['rackspace_postgresql']['config_pgtune']['max_connections']
  if max_connections.match(/\A[1-9]\d*\Z/).nil?
    Chef::Application.fatal!([
        "Bad value (#{max_connections})",
        'for node[\'rackspace_postgresql\'][\'config_pgtune\'][\'max_connections\'] attribute.',
...
if node['rackspace_postgresql'].attribute?('config_pgtune') && node['rackspace_postgresql']['config_pgtune'].attribute?('total_memory')
  total_memory = node['rackspace_postgresql']['config_pgtune']['total_memory']
  if total_memory.match(/\A[1-9]\d*kB\Z/).nil?
    Chef::Application.fatal!([
        "Bad value (#{total_memory})",
        'for node[\'rackspace_postgresql\'][\'config_pgtune\'][\'total_memory\'] attribute.',
...
node.default['rackspace_postgresql']['config']['max_connections'] = con
...
if mem >= 256
  ...
  node.default['rackspace_postgresql']['config']['shared_buffers'] = binaryround(shared_buffers * 1024 * 1024)
  ...
  node.default['rackspace_postgresql']['config']['effective_cache_size'] = binaryround(effective_cache_size * 1024 * 1024)
  ...
  node.default['rackspace_postgresql']['config']['work_mem'] = binaryround(work_mem * 1024 * 1024)
  ...
  node.default['rackspace_postgresql']['config']['maintenance_work_mem'] = binaryround(maintenance_work_mem * 1024 * 1024)
...
node.default['rackspace_postgresql']['config']['checkpoint_segments'] = checkpoint_segments
...
node.default['rackspace_postgresql']['config']['checkpoint_completion_target'] = checkpoint_completion_target
...
if node['rackspace_postgresql']['version'].to_f < 9.1
  wal_buffers = 512 * checkpoint_segments
  # The pgtune seems to use 1kB units for wal_buffers
  node.default['rackspace_postgresql']['config']['wal_buffers'] = binaryround(wal_buffers * 1024)
else
  node.default['rackspace_postgresql']['config']['wal_buffers'] = '-1'
end
...
node.default['rackspace_postgresql']['config']['default_statistics_target'] = default_statistics_target
